...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles)
    {
        ...
        //pose
        vehicle_ros->true_pose = nh_private_.advertise<geometry_msgs::PoseStamped>("true_pose", 100);
...
void AirsimROSWrapper::publish_vehicle_state()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
    {
        ...
        //publish pose
        Pose Vehicle_pose = airsim_client_->simGetVehiclePose(vehicle_ros->vehicle_name);
         geometry_msgs::PoseStamped pose;
        pose.header.stamp = ros::Time::now();
        pose.header.frame_id ="map";

        tf::Transform new_transform;
        new_transform.setOrigin(tf::Vector3(Vehicle_pose.position[0],Vehicle_pose.position[1],Vehicle_pose.position[2]));
        tf::Quaternion quaternion(Vehicle_pose.orientation.x(), Vehicle_pose.orientation.y(), Vehicle_pose.orientation.z(), Vehicle_pose.orientation.w());
        new_transform.setRotation(quaternion);
        tf::poseTFToMsg(new_transform, pose.pose);
        vehicle_ros->true_pose.publish(pose);
